...
function mcstats_table() {
	...
	$string .= '<table style="float: '.$options['stat_float'].'; max-width: '.$maxwidth.'" id="stat-table"><tr><td>';

	$blockitemid = get_post_meta($post_id, 'blockitemid', true);
	if ($blockitemid != '') {
		$string .= '<br /><span class="stat-name">'.$options['lang_block_id'].'</span>: '.$blockitemid.'';
	}

	$atkstr = get_post_meta($post_id, 'attack_strength', true);
	if ($atkstr != '') {
		$string .= '<br /><span class="stat-name">'.$options['lang_atk_str'].'</span>: '.$atkstr.'';
	}

	$health = get_post_meta($post_id, 'health', true);
	if ($health != '') {
		$string .= '<br /><span class="stat-name">'.$options['lang_health'].':</span>';
		if ($odd = $health%2) {
			$health = (($health - 1) / 2);
			$string .= '<span style="display: inline-block; background: url('.$plugin_url.'images/heart.png) repeat-x; height: 18px; width: ' . (18*$health) . 'px" title="'.($health*2+1).' '.$options['lang_health'].'" alt="'.($health*2+1).' '.$options['lang_health'].'"></span>';
			$string .= '<span style="display: inline-block; background: url('.$plugin_url.'images/heart_half.png) repeat-x; height: 18px; width: 18px" title="'.($health*2+1).' '.$options['lang_health'].'" alt="'.($health*2+1).' '.$options['lang_health'].'"></span>';
		}
		else {
			$string .= '<span style="display: inline-block; background: url('.$plugin_url.'images/heart.png) repeat-x; height: 18px; width: ' . (18*$health) . 'px" title="'.$health.' '.$options['lang_health'].'" alt="'.$health.' '.$options['lang_health'].'"></span>';
		}
	}
	$healsfor = get_post_meta($post_id, 'heals_for', true);
	if ($healsfor != '') {
		$string .= '<br /><span class="stat-name">'.$options['lang_heals_for'].':</span>';
		if ($odd = $healsfor%2) {
			$healsfor = (($healsfor - 1) / 2);
			$string .= '<span style="display: inline-block; background: url('.$plugin_url.'images/heart.png) repeat-x; height: 18px; width: ' . (18*$healsfor) . 'px" title="'.($healsfor*2+1).' Health" alt="'.($healsfor*2+1).' Health"></span>';
			$string .= '<span style="display: inline-block; background: url('.$plugin_url.'images/heart_half.png) repeat-x; height: 18px; width: 18px" title="'.($healsfor*2+1).' Health" alt="'.($healsfor*2+1).' Health"></span>';
		}
		else {
			$string .= '<span style="display: inline-block; background: url('.$plugin_url.'images/heart.png) repeat-x; height: 18px; width: ' . (18*$healsfor/2) . 'px" title="'.$healsfor.' Health" alt="'.$healsfor.' Health"></span>';
		}
	}
		$damages = get_post_meta($post_id, 'damages_for', true);
	if ($damages != '') {
		$string .= '<br /><span class="stat-name">'.$options['lang_damages_for'].':</span>';
		if ($odd = $damages%2) {
			$damages = (($damages - 1) / 2);
			$string .= '<span style="display: inline-block; background: url('.$plugin_url.'images/heart-g.png) repeat-x; height: 18px; width: ' . (18*$damages) . 'px" title="'.$options['lang_damages_for'].' '.($damages*2+1).'" alt="'.$options['lang_damages_for'].' '.($damages*2+1).'"></span>';
			$string .= '<span style="display: inline-block; background: url('.$plugin_url.'images/heart_half-g.png) repeat-x; height: 18px; width: 18px" title="'.$options['lang_damages_for'].' '.($damages*2+1).'" alt="'.$options['lang_damages_for'].' '.($damages*2+1).'"></span>';
		}
		else {
			$string .= '<span style="display: inline-block; background: url('.$plugin_url.'images/heart-g.png) repeat-x; height: 18px; width: ' . (18*$damages/2) . 'px" title="'.$options['lang_damages_for'].' '.$damages.'" alt="'.$options['lang_damages_for'].' '.$damages.'"></span>';
		}
	}
	$armor = get_post_meta($post_id, 'points_of_protection', true);
	if ($armor != '') {
		$string .= '<br /><span class="stat-name">'.$options['lang_armor'].':</span> <img src="'.$plugin_url.'/images/pop'.$armor.'.png" title="'.$armor.' '.$options['lang_armor'].'" alt="'.$armor.' '.$options['lang_armor'].'" />';
	}
	$durab = get_post_meta($post_id, 'durability', true);
	if ($durab != '') {
		$string .= '<br /><span class="stat-name">'.$options['lang_durability'].'</span>: '.$durab.'';
	}
	$hunger_restoration = get_post_meta($post_id, 'hunger_restoration', true);
	if ($hunger_restoration != '') {
		$string .= '<br /><span class="stat-name">'.$options['lang_hunger_rest'].':</span> <span style="display: inline-block; background: url('.$plugin_url.'images/hung1.png) repeat-x; height: 18px; width: ' . (18*$hunger_restoration) . 'px" title="'.$hunger_restoration.' '.$options['lang_hunger_rest'].'" alt="'.$hunger_restoration.' '.$options['lang_hunger_rest'].'"></span>';
	}
	$stackable = get_post_meta($post_id, 'stackable', true);
	if ($stackable != '') {
		if ($stackable > 1) {
			$string .= '<br /><span class="stat-name">'.$options['lang_stackable'].'</span>: '.$options['lang_yes'].' ('.$stackable.')';
		} else {
			$string .= '<br /><span class="stat-name">'.$options['lang_stackable'].'</span>: '.$options['lang_no'].'';
		}
	}
	$string .= '</td></tr>';
	$string .= '</table>';
...
function mcsources_table() {
	...
	if (is_array($sources) && count($sources) > 0 && $sources[0] != '') {
		...
		foreach ($sources As $v) {
			if (is_numeric($v)) {
				$string .= '<a class="sourcelink" href="'.get_permalink($v).'" title="'.get_the_title($v).'" alt="'.get_the_title($v).'">'.get_the_post_thumbnail($v, 'recipe-table').'</a>';
			}
		}
		$string .= '</td></tr></table>';
	}
